...
const serverApp = await createHonoServer({
  app,
  defaultLogger: false,
});

// For compatibility: export a callable handler that also exposes a `.fetch` property.
const handler = serverApp.fetch.bind(serverApp);
(handler as any).fetch = handler;
